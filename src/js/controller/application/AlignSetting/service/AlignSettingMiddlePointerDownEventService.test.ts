...
import { execute } from "./AlignSettingMiddlePointerDownEventService";

describe("AlignSettingMiddlePointerDownEventService", () => {
    ...
    beforeEach(() => {
        ...
        mockExternalAlignInstance = {
            middle: vi.fn().mockResolvedValue(undefined)
        };
    ...
    describe("正常処理", () => {
        it("左クリック時に中間揃えが実行される", async () => {
            ...
            expect(mockExternalAlignInstance.middle).toHaveBeenCalled();
        });
    ...
    describe("非同期処理", () => {
        it("middle メソッドが非同期で実行される", async () => {
            let methodCalled = false;
            mockExternalAlignInstance.middle.mockImplementation(async () => {
