...
describe("ReferenceSettingYFocusOutEventUseCase", () => {
    ...
    describe("正常系", () => {
        ...
        test("単一選択時にExternalReferenceのsetYが呼ばれる", async () => {
        ...
        test("正の小数点値が正しく処理される", async () => {
            ...
            mockReferenceSetting.beforeY = 400;
    ...
    describe("早期リターンの条件", () => {
        ...
        test("beforeYと同じ値の場合、早期リターンする", async () => {
            mockClamp.mockReturnValue(100);
            mockReferenceSetting.beforeY = 100;
    ...
    describe("非同期処理の確認", () => {
        ...
        test("ExternalReference.setYが非同期で実行される", async () => {
            ...
            expect(mockExternalReference.setY).toHaveBeenCalledWith(700);
        });

        test("ExternalReference.setYでエラーが発生してもthrowされない", async () => {
    ...
    describe("単一選択と複数選択の分岐", () => {

        test("単一選択の場合、ExternalReferenceが使用される", async () => {
            mockClamp.mockReturnValue(400);
            mockReferenceSetting.beforeY = 100;
            mockMovieClip.isSingleSelectedOfDisplayObject.mockReturnValue(true);
            ...
            expect(mockExternalReference.setY).toHaveBeenCalledWith(400);
            expect(mockScreenReferencePointDeployElementUseCase).not.toHaveBeenCalled();
        });

        test("複数選択の場合、screenReferencePointDeployElementUseCaseが使用される", async () => {
            mockClamp.mockReturnValue(500);
            mockReferenceSetting.beforeY = 100;
            mockMovieClip.isSingleSelectedOfDisplayObject.mockReturnValue(false);
    ...
    describe("clamp処理の確認", () => {
        ...
        test("計算結果が範囲内の場合、そのまま使用される", async () => {
            ...
            expect(mockExternalReference.setY).toHaveBeenCalledWith(250);
        });

        test("clampによる値の制限が正しく反映される", async () => {
