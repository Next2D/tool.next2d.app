...
describe("ColorSettingAlphaMultiplierFocusOutEventUseCase", () => {
    ...
    describe("基本動作", () => {
        ...
        it("非同期処理が完了するまで待機する", async () => {
            let resolved = false;
            mockColorSettingAlphaMultiplierUpdateValueUseCase.mockImplementation(async () => {
    ...
    describe("値の解析とクランプ", () => {
        ...
        it("小数点を含む値が正しく解析される", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(75.5, 0, 100);
        });
        ...
        it("負の値が0にクランプされる (type='range'は自動正規化)", async () => {
            mockElement.type = "range";
            ...
            // type="range"の場合、-10は自動的に0に正規化される
            expect(mock$clamp).toHaveBeenCalledWith(0, 0, 100);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(0);
        });

        it("100を超える値が100にクランプされる (type='range'は自動正規化)", async () => {
            mockElement.type = "range";
            ...
            // type="range"の場合、150は自動的に100に正規化される
            expect(mock$clamp).toHaveBeenCalledWith(100, 0, 100);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(100);
        });

        it("type='number'で負の値の場合はparseFloatした値がclampに渡される", async () => {
            ...
            // type="number"の場合、-10はそのまま渡され、clampで0になる
            expect(mock$clamp).toHaveBeenCalledWith(-10, 0, 100);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(0);
        });

        it("type='number'で100を超える値の場合はparseFloatした値がclampに渡される", async () => {
            mockElement.type = "number";
            mockElement.value = "150";
            mock$clamp.mockReturnValue(100);
            ...
            // type="number"の場合、150はそのまま渡され、clampで100になる
            expect(mock$clamp).toHaveBeenCalledWith(150, 0, 100);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(100);
        });

        it("空文字列の場合 (type='range'は50に正規化)", async () => {
            mockElement.type = "range";
            mockElement.value = "";
            mock$clamp.mockReturnValue(50);
            ...
            expect(mock$clamp).toHaveBeenCalledWith(50, 0, 100);
        });

        it("不正な文字列の場合 (type='range'は50に正規化)", async () => {
            mockElement.type = "range";
    ...
    describe("統合シナリオ", () => {
        it("完全なフォーカスアウトフロー", async () => {
            ...
            // 3. 値がクランプされる
            expect(mock$clamp).toHaveBeenCalledWith(80, 0, 100);

            // 4. 値が更新される
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(80);
        });

        it("範囲外の値が正しく処理される (type='range'は自動正規化)", async () => {
            mockElement.type = "range";
            mockElement.value = "150";
            mock$clamp.mockReturnValue(100);
        ...
        it("負の値が正しく処理される (type='range'は自動正規化)", async () => {
            mockElement.type = "range";
            mockElement.value = "-50";
            mock$clamp.mockReturnValue(0);
            ...
            // type="range"の場合、-50は自動的に0に正規化される
            expect(mock$clamp).toHaveBeenCalledWith(0, 0, 100);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(0);
        });

        it("updateValueUseCaseがエラーを投げても伝播する", async () => {
            mockColorSettingAlphaMultiplierUpdateValueUseCase.mockRejectedValue(new Error("Test Error"));
    ...
    describe("エッジケース", () => {
        ...
        it("value = '0.1'の場合", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(0.1, 0, 100);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(0.1);
        });
        ...
        it("value = '99.99'の場合", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(99.99, 0, 100);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(99.99);
        });
        ...
        it("value = '50.5'の場合", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(50.5, 0, 100);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(50.5);
        });

        it("value = '-1'の場合 (0にクランプ)", async () => {
            mockElement.value = "-1";
            mock$clamp.mockReturnValue(0);
        ...
        it("value = '101'の場合 (100にクランプ)", async () => {
            mockElement.value = "101";
            mock$clamp.mockReturnValue(100);
    ...
    describe("HTMLInputElementの動作", () => {
        it("input要素のvalueプロパティが正しく読み取られる", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(42, 0, 100);
        });

        it("type='range'の要素で動作する", async () => {
            mockElement.type = "range";
            ...
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(55);
        });

        it("type='number'の要素でも動作する", async () => {
            mockElement.type = "number";
            ...
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(88);
        });
    });
    ...
    describe("FocusEventの処理", () => {
        ...
        it("複数回実行してもそれぞれ正しく処理される", async () => {
            ...
            await execute(mockEvent);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(10);

            mockElement.value = "20";
            mock$clamp.mockReturnValue(20);
            await execute(mockEvent);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(20);

            mockElement.value = "30";
            mock$clamp.mockReturnValue(30);
            await execute(mockEvent);
            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledWith(30);

            expect(mockColorSettingAlphaMultiplierUpdateValueUseCase).toHaveBeenCalledTimes(3);
        });
    });
    ...
    describe("parseFloatの動作", () => {
        it("parseFloatが内部で使用される", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(75.75, 0, 100);
        });
