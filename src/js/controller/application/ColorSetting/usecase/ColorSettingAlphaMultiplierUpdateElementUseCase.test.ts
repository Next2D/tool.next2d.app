...
describe("ColorSettingAlphaMultiplierUpdateElementUseCase", () => {
    ...
    describe("基本動作", () => {
        ...
        it("alphaが正しく更新される(100の場合)", async () => {
        ...
        it("alphaが正しく更新される(50の場合)", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);

            await execute(50);

            // alpha / 100 = 50 / 100 = 0.5
            expect(mockCharacter.colorTransform[3]).toBe(0.5);
        });

        it("alphaが正しく更新される(0の場合)", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);
        ...
        it("canvas要素のopacityが更新される", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);
        ...
        it("canvas要素が存在しない場合でもエラーにならない", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);
    ...
    describe("colorTransformの配列インデックス", () => {
        it("colorTransform[3]がアルファ値として更新される", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);
        ...
        it("colorTransformの他の要素に影響しない", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);
    ...
    describe("エラーケース", () => {
        ...
        it("複数エラーが混在する場合、有効なものだけ更新される", async () => {
            ...
            // 2つ目だけ更新される
            expect(mockCharacter2.colorTransform[3]).toBe(0.5);
        });
    });
    ...
    describe("canvasのquerySelector", () => {
        it("canvas要素が見つかる場合、opacityが設定される", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);
        ...
        it("canvas要素が見つからない場合でもエラーにならない", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);
            mockNode.querySelector.mockReturnValue(null);
        ...
        it("querySelectorが複数回呼ばれる(複数depth)", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1, 2, 3]]]);
        ...
        it("character.alphaが正しく参照される", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);
    ...
    describe("統合シナリオ", () => {
        ...
        it("完全な更新フロー: 複数選択", async () => {
            ...
            // 全てのキャラクターが更新される
            expect(mockCharacter.colorTransform[3]).toBe(0.6);
            expect(mockCharacter2.colorTransform[3]).toBe(0.6);
            expect(mockCharacter3.colorTransform[3]).toBe(0.6);
        });
        ...
        it("境界値: alpha = 0とalpha = 100を連続して更新", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);
    ...
    describe("エッジケース", () => {
        ...
        it("alpha値が非常に小さい場合", async () => {
            mockMovieClip.selectedDepths = new Map([[0, [1]]]);

            await execute(0.01);

            // 0.01 / 100 = 0.0001
            expect(mockCharacter.colorTransform[3]).toBeCloseTo(0.0001, 5);
        });
