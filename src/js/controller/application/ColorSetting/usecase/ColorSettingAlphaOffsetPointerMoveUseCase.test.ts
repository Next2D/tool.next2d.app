...
const mockColorSettingAlphaOffsetUpdateElementUseCase = vi.fn();
...
vi.mock("./ColorSettingAlphaOffsetUpdateElementUseCase", () => ({
    execute: (movieClip: any, value: number) => mockColorSettingAlphaOffsetUpdateElementUseCase(movieClip, value)
}));
...
// 動的インポート
const { execute } = await import("./ColorSettingAlphaOffsetPointerMoveUseCase");

describe("ColorSettingAlphaOffsetPointerMoveUseCase", () => {
    ...
    describe("基本動作", () => {
        ...
        it("colorSettingAlphaOffsetUpdateElementUseCaseが呼ばれる", async () => {
            mockElement.value = "50";
            const mockEvent = createMockEvent(10, mockElement);
            ...
            expect(mockColorSettingAlphaOffsetUpdateElementUseCase).toHaveBeenCalledWith(mockMovieClip, 60);
        });
    });
    ...
    describe("早期リターン条件", () => {
        ...
        it("colorSettingStateが'up'の場合は何もしない", async () => {
            ...
            expect(mockColorSettingAlphaOffsetUpdateElementUseCase).not.toHaveBeenCalled();
    ...
    describe("値の計算", () => {
        ...
        it("負のmovementXで値が減少する", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(40, -255, 255);
        ...
        it("$clampが0から100の範囲でクランプする", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(55, -255, 255);
        });
        ...
        it("100を超える値が100にクランプされる", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(105, -255, 255);
        ...
        it("0未満の値が0にクランプされる", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(-5, -255, 255);
    ...
    describe("parseFloatの動作", () => {
        it("小数点を含む値が正しくパースされる", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(55.5, -255, 255);
        });
        ...
        it("文字列の数値が正しくパースされる", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(35, -255, 255);
        });
    });
    ...
    describe("エッジケース", () => {
        it("movementXが非常に大きい値の場合", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(1050, -255, 255);
            expect(mockElement.value).toBe("100");
        });
        ...
        it("movementXが非常に小さい値の場合", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(-950, -255, 255);
            expect(mockElement.value).toBe("0");
        });
        ...
        it("要素のvalueが初期状態で空文字列の場合", async () => {
            ...
            expect(clampArgs[1]).toBe(-255);
            expect(clampArgs[2]).toBe(255);
            // 最終的な値は有効な数値になる
