...
const mockColorSettingAlphaOffsetPointerMoveUseCase = vi.fn();
...
vi.mock("./ColorSettingAlphaOffsetPointerMoveUseCase", () => ({
    execute: mockColorSettingAlphaOffsetPointerMoveUseCase
}));

vi.mock("./ColorSettingAlphaOffsetUpdateValueUseCase", () => ({
    execute: async (value: number) => mockColorSettingAlphaOffsetUpdateValueUseCase(value)
}));

// 動的インポート
const { execute } = await import("./ColorSettingAlphaOffsetPointerUpUseCase");

describe("ColorSettingAlphaOffsetPointerUpUseCase", () => {
    ...
    describe("イベントリスナーの削除", () => {
        it("POINTER_MOVEイベントリスナーが削除される", async () => {
            ...
            expect(mockElement.removeEventListener).toHaveBeenCalledWith(
                ...
                mockColorSettingAlphaOffsetPointerMoveUseCase
            );
        });
    ...
    describe("値の処理", () => {
        ...
        it("小数点を含む値(切り捨て)", async () => {
            ...
            // 99.9 | 0 = 99
            expect(mockColorSettingAlphaOffsetUpdateValueUseCase).toHaveBeenCalledWith(99);
        });
    ...
    describe("ビット演算(| 0)の動作", () => {
        it("正の小数が切り捨てられる", async () => {
            ...
            expect(mockColorSettingAlphaOffsetUpdateValueUseCase).toHaveBeenCalledWith(45);
        });
    ...
    describe("実行順序", () => {
        it("処理が正しい順序で実行される", async () => {
            ...
            mockColorSettingAlphaOffsetUpdateValueUseCase.mockImplementation(() => callOrder.push("updateValue"));
    ...
    describe("統合シナリオ", () => {
        it("完全なポインターアップフロー", async () => {
            ...
            // 6. 値更新
            expect(mockColorSettingAlphaOffsetUpdateValueUseCase).toHaveBeenCalledWith(85);
        ...
        it("エラーケース: targetがnullの場合でも状態は変更される", async () => {
            ...
            expect(mock$setCursor).toHaveBeenCalledWith("auto");
            // 他の処理は実行されない
            expect(mockColorSettingAlphaOffsetUpdateValueUseCase).not.toHaveBeenCalled();
        });
    ...
    describe("エッジケース", () => {
        it("pointerId = 0 (マウス)", async () => {
            ...
            expect(mockElement.releasePointerCapture).toHaveBeenCalledWith(0);
            expect(mockColorSettingAlphaOffsetUpdateValueUseCase).toHaveBeenCalledWith(60);
        });
        ...
        it("pointerId = 1 (最初のタッチ)", async () => {
            ...
            expect(mockElement.releasePointerCapture).toHaveBeenCalledWith(1);
            expect(mockColorSettingAlphaOffsetUpdateValueUseCase).toHaveBeenCalledWith(70);
        });
