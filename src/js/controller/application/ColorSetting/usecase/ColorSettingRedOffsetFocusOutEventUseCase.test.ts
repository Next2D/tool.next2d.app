...
const mockColorSettingRedOffsetUpdateValueUseCase = vi.fn();
...
vi.mock("./ColorSettingRedOffsetUpdateValueUseCase", () => ({
    execute: async (red: number) => mockColorSettingRedOffsetUpdateValueUseCase(red)
}));

// 動的インポート
const { execute } = await import("./ColorSettingRedOffsetFocusOutEventUseCase");

describe("ColorSettingRedOffsetFocusOutEventUseCase", () => {
    ...
    beforeEach(() => {
        ...
        mockColorSettingRedOffsetUpdateValueUseCase.mockResolvedValue(undefined);
    });
    ...
    describe("基本動作", () => {
        ...
        it("colorSettingRedOffsetUpdateValueUseCaseが呼ばれる", async () => {
            mock$clamp.mockReturnValue(0);

            await execute(mockEvent);

            expect(mockColorSettingRedOffsetUpdateValueUseCase).toHaveBeenCalledWith(0);
            expect(mockColorSettingRedOffsetUpdateValueUseCase).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe("値の解析とクランプ", () => {
        ...
        it("-255の値が正しく処理される (type='number'で負の値を許容)", async () => {
            ...
            expect(mockColorSettingRedOffsetUpdateValueUseCase).toHaveBeenCalledWith(-255);
        });
        ...
        it("type='range'の場合、最大値を超える値は最大値に正規化される", async () => {
            ...
            expect(mock$clamp).toHaveBeenCalledWith(100, -255, 255);
            expect(mockColorSettingRedOffsetUpdateValueUseCase).toHaveBeenCalledWith(100);
        });
    });
});
