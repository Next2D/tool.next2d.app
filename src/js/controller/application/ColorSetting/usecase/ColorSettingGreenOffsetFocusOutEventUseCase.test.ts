...
// 動的インポート
const { execute } = await import("./ColorSettingGreenOffsetFocusOutEventUseCase");

describe("ColorSettingGreenOffsetFocusOutEventUseCase", () => {
    ...
    describe("基本動作", () => {
        ...
        it("colorSettingGreenOffsetUpdateValueUseCaseが呼ばれる", async () => {
            ...
            expect(mockColorSettingGreenOffsetUpdateValueUseCase).toHaveBeenCalledWith(0);
            expect(mockColorSettingGreenOffsetUpdateValueUseCase).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe("値の解析とクランプ", () => {
        ...
        it("-255の値が正しく処理される (type='number'で負の値を許容)", async () => {
            ...
            expect(mockColorSettingGreenOffsetUpdateValueUseCase).toHaveBeenCalledWith(-255);
        });
