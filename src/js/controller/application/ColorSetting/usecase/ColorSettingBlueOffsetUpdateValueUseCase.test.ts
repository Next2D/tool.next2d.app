...
describe("ColorSettingBlueOffsetUpdateValueUseCase", () => {
    ...
    beforeEach(() => {
        ...
        mockExternalCharacter.setBlueOffset.mockResolvedValue(undefined);
    });
    ...
    describe("基本動作", () => {
        ...
        it("setBlueOffsetが正しい値で呼ばれる", async () => {
            await execute(75);

            expect(mockExternalCharacter.setBlueOffset).toHaveBeenCalledWith(75);
        });

        it("blueが更新前の値に戻されてから新しい値が設定される", async () => {
            colorSetting.beforeValue = 10;
            mockCharacter.colorTransform[6] = 50;

            await execute(100);

            expect(mockCharacter.colorTransform[6]).toBe(10);
            expect(mockExternalCharacter.setBlueOffset).toHaveBeenCalledWith(100);
        });
    });
});
