...
vi.mock("./ColorSettingBlueMultiplierUpdateElementUseCase", () => ({
    execute: (movieClip: any, value: number) => mockColorSettingBlueMultiplierUpdateElementUseCase(movieClip, value)
}));
...
describe("ColorSettingBlueMultiplierPointerMoveUseCase", () => {
    ...
    describe("基本動作", () => {
        ...
        it("colorSettingBlueMultiplierUpdateElementUseCaseが呼ばれる", async () => {
            mockElement.value = "50";
            const mockEvent = createMockEvent(10, mockElement);
            ...
            expect(mockColorSettingBlueMultiplierUpdateElementUseCase).toHaveBeenCalledWith(mockMovieClip, 60);
        });
    });
});
