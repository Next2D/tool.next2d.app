...
const mockColorSettingRedMultiplierUpdateElementUseCase = vi.fn();
...
vi.mock("./ColorSettingRedMultiplierUpdateElementUseCase", () => ({
    execute: (movieClip: MovieClip, value: number) => mockColorSettingRedMultiplierUpdateElementUseCase(movieClip, value)
}));

const { execute } = await import("./ColorSettingRedMultiplierPointerMoveUseCase");

describe("ColorSettingRedMultiplierPointerMoveUseCase", () => {
    ...
    describe("基本動作", () => {
        ...
        it("movementXが存在する場合は処理が実行される", () => {
            ...
            expect(mockColorSettingRedMultiplierUpdateElementUseCase).toHaveBeenCalled();
        });
        ...
        it("カラー設定の状態がupの場合は処理をスキップ", () => {
            ...
            expect(mockColorSettingRedMultiplierUpdateElementUseCase).not.toHaveBeenCalled();
        });
    });
});
