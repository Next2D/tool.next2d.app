...
vi.mock("./ColorSettingRedOffsetPointerMoveUseCase", () => ({
    execute: vi.fn()
}));

vi.mock("./ColorSettingRedOffsetPointerUpUseCase", () => ({
    execute: vi.fn()
}));

const { execute } = await import("./ColorSettingRedOffsetPointerDownUseCase");
...
describe("ColorSettingRedOffsetPointerDownUseCase", () => {
