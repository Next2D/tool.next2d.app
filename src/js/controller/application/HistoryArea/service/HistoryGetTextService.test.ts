...
describe("HistoryGetTextServiceTest", () =>
{
    test("execute test", () =>
    {
        ...
        expect(execute($LIBRARY_OVERWRITE_IMAGE_COMMAND)).toBe("「%s1」の画像を上書き");
        expect(execute($LIBRARY_ADD_NEW_VIDEO_COMMAND)).toBe("動画「%s1」を取り込み");
        expect(execute($LIBRARY_OVERWRITE_VIDEO_COMMAND)).toBe("「%s1」の動画を上書き");
        expect(execute($LIBRARY_ADD_NEW_SOUND_COMMAND)).toBe("音声「%s1」を取り込み");
        expect(execute($LIBRARY_OVERWRITE_SOUND_COMMAND)).toBe("「%s1」の音声を上書き");
