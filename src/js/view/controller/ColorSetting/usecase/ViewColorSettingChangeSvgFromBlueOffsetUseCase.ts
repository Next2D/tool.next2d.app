...
/**
 * @description 選択中のElementの青色値の更新に合わせてsvgを更新する
 *              Update the svg according to the change of the blue value of the selected Element
 *
...
export const execute = (
    ...
    let blueFilter = document.getElementById(`fx-b-${layer.id}-${character.id}`);
    if (!blueFilter) {
        ...
        // 再取得
        blueFilter = document.getElementById(`fx-b-${layer.id}-${character.id}`);
    }
    ...
    blueFilter.setAttribute("intercept", `${character.colorTransform[6] / 255}`);
};