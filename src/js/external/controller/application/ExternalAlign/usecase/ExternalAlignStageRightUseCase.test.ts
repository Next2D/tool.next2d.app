...
import { execute } from "./ExternalAlignStageRightUseCase";

describe("ExternalAlignStageRightUseCase", () => {
    ...
    it("ステージの右端に合わせて位置が調整される", async () => {
        await execute(mockWorkSpace, mockMovieClip);
        // rightX = 800, dx = 800 + (100 - 120) = 780
        expect(mockExternalCharacterInstance.setX).toHaveBeenCalledWith(780);
    });
});
