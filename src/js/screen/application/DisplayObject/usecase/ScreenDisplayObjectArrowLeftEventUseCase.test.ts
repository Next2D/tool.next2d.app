...
// 動的インポート
const { execute } = await import("./ScreenDisplayObjectArrowLeftEventUseCase");
const { transformSetting } = await import("@/controller/domain/model/TransformSetting");

describe("ScreenDisplayObjectArrowLeftEventUseCase", () => {
    ...
    describe("基本的な左方向移動", () => {
        it("Shiftキーなしで1ピクセル左に移動する", async () => {
        ...
        it("Shiftキーありで10ピクセル左に移動する", async () => {
        ...
        it("X方向の移動量は常に負である", async () => {
            const mockEvent = createMockEvent(false);
            mockWorkSpace.scale = 1;

            await execute(mockEvent);

            expect(transformSetting.x).toBeLessThan(0);
            // expect(transformSetting.beforeX).toBe(50);  // Arrow events don\'t set beforeX/Y
        });
    });
    ...
    describe("transformSettingの設定", () => {
        ...
        it("transformSetting.xが正しく計算される（負の値）", async () => {
            const mockEvent = createMockEvent(false);
            mockWorkSpace.scale = 1;
    ...
    describe("エッジケース", () => {
        it("scale=0の場合、移動量も0になる", async () => {
            ...
            expect(transformSetting.x).toBe(-0); // -1 * 0 = -0 (JavaScript negative zero)
            // expect(transformSetting.beforeX).toBe(50);  // Arrow events don\'t set beforeX/Y
            expect(mockScreenDisplayObjectUpdateSelectedValueService).toHaveBeenCalled();
        });
        ...
        it("scale=0でShiftキーありの場合も移動量は0", async () => {
            ...
            expect(transformSetting.x).toBe(-0); // -10 * 0 = -0 (JavaScript negative zero)
            // expect(transformSetting.beforeX).toBe(50);  // Arrow events don\'t set beforeX/Y
            expect(mockScreenDisplayObjectUpdateSelectedValueService).toHaveBeenCalled();
        });
    ...
    describe("統合シナリオ", () => {
        ...
        it("完全な左移動フロー（Shiftキーあり、scale=2）", async () => {
            const mockEvent = createMockEvent(true);
            mockWorkSpace.scale = 2;
    ...
    describe("左方向特有のテスト", () => {
        ...
        it("Shiftキーありでも移動方向が負（左）である", async () => {
            const mockEvent = createMockEvent(true);
            mockWorkSpace.scale = 1;
